...
describe("POST api/v1/sessions", () => {
  describe("Anonymous user", () => {
    test("With incorrent `email` but corrent `password`", async () => {
    ...
    test("With correct `email` but incorrent `password`", async () => {
      await orchestrator.createUser({
    ...
    test("With incorrect `email` and incorrent `password`", async () => {
      await orchestrator.createUser();
    ...
    test("With correct `email` and corrent `password`", async () => {
      ...
      const parsedSetCookie = setCookieParser(response, {
        map: true, // retorna o obj diretamente
      });
